		...
		if ( checkPermissions() ) {
			...
			Request uploadRequest = Request.newUploadPhotoRequest(Session.getActiveSession(), img, new Request.Callback() {
				@Override public void onCompleted(Response response) {
					Toast.makeText(LoginActivity.this, "Photo uploaded successfully", Toast.LENGTH_LONG).show();
				}
			});
	...
	public void postStatusMessage() {
		if ( checkPermissions() ) {
			Request request = Request.newStatusUpdateRequest(Session.getActiveSession(), message, new Request.Callback() {
				@Override public void onCompleted(Response response) {
					if ( response.getError() == null ) Toast.makeText(LoginActivity.this, "Status updated successfully", Toast.LENGTH_LONG).show();
				}
			});
