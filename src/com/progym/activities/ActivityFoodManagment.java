               ...
               @Override public boolean onDrag(View v, DragEvent event) {
                    ...
                    switch (event.getAction()) {
                         case DragEvent.ACTION_DROP:
                              ...
                              Cursor cursor = DataBaseUtils.getByGroupNameAndIngridientName(tag);
                              Ingridient ingridient = null;
                              while ( cursor.moveToNext() ) {
                                   ingridient = new Ingridient(getApplicationContext());
                                   ingridient.protein = cursor.getInt(1);
                                   ingridient.carbohydrates = cursor.getInt(2);
                                   ingridient.fat = cursor.getInt(3);
                                   ingridient.kkal = cursor.getInt(4);
                                   ingridient.name = cursor.getString(5);
                                   ingridient.groupName = cursor.getString(6);
                              ...
                              if ( null == ingridient ) {
                                   Utils.log("NE NAWEL V BASE");
                                   return true;
                              }

                              OneProductOnPlateView view = OneProductOnPlateView_.build(getApplicationContext());
                              view.ivProduct.setImageResource(Utils.getImageIdByGroupPositionInExpListView(FoodTypeExpListViewFragment.FOOD_CATALOGUE));
                              view.twProductDescription.setText(ingridient.name + " : " + CURRENT_MEAL.date);
     ...
     private void putProductsOnPlate() {
          ...
          if ( null != CURRENT_MEAL ) {
               ...
               for ( Ingridient ingridient : ingridients ) {
                    ...
                    view.twProductDescription.setText(ingridient.name + " meal date " + CURRENT_MEAL.date);
                    llAlreadyOnPlate.addView(view);
               }
          }
     }
     ...
     private void createProductOnPlate(Meal meal) {
          ...
          itemView.setOnClickListener(new OnClickListener() {
               ...
               @Override public void onClick(View v) {
                    ...
                    if ( null != ingridients ) {
                         for ( Ingridient ingridient : ingridients ) {
                              Utils.log(String.format("Ingridient -> protein:%s , carbs:%s , name:%s", ingridient.protein, ingridient.carbohydrates, ingridient.name));
                         }
                    }
