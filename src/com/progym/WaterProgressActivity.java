...
public class WaterProgressActivity extends ProgymSuperProgressActivity implements IProgressActivity {
	...
	@Override public void updateFragment(int fragmentType) {
		...
		switch (fragmentType) {
			case DAILY:
				...
				((ProgressViewPagerAdapter) viewPager.getAdapter()).bargraph.setBarData(date, DataBaseUtils.getWaterUserShouldConsumePerDay(), DataBaseUtils.getConsumedPerDay(date));
				break;
			case MONTHLY:
				...
				alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
					@Override public void onClick(DialogInterface dialog, int whichButton) {
						...
						// Add to expandable list view ready meal date
						((ProgressViewPagerAdapter) viewPager.getAdapter()).linegpah.setLineData2(DataBaseUtils.getAllWaterConsumedInMonth(Utils.dateFormat.format(SELECTED_DATE)), month.getCurrentItem());
					}
				});
